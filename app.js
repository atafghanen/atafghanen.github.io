const DEFAULT_DATA={
  settings:{instagram:"https://instagram.com/",tiktok:"https://tiktok.com/",email:"info@ateveningelegance.com",whatsapp:"491700000000",logo:"assets/logo.gif"},
  products:[
...
let lang="de", cart=JSON.parse(localStorage.getItem("atEECart")||"[]"), currentCategory="all";
...
function categoryLabel(cat){
  const map={
    ...
    "Neue Kategorie":{de:"Neue Kategorie",en:"New Category",ps:"نوې کټګوري",fa:"دسته‌بندی جدید"}
  };
...
function uiText(key){
  const m={
    ...
    detail:{de:"Dieses Design kann aus dem Lager bestellt oder individuell nach Maß angefertigt werden.",
...
function save(){localStorage.setItem("atEEData",JSON.stringify(data));localStorage.setItem("atEECart",JSON.stringify(cart))}
function applyBranding(){
  const logo=data.settings.logo||"assets/logo.gif";
  document.querySelectorAll(".logo-spin,.footer-logo,.hero-logo").forEach(img=>{img.src=logo;});
  const wa=data.settings.whatsapp||"";
  window.AT_WHATSAPP=wa.replace(/[^0-9]/g,"");
}
function applyLang(){
  applyBranding();
  document.documentElement.lang=lang;document.documentElement.dir=(lang==="ps"||lang==="fa")?"rtl":"ltr";localStorage.setItem("atEELang",lang);
  $$("[data-i18n]").forEach(el=>el.innerHTML=t(el.dataset.i18n));
  $$("[data-placeholder]").forEach(el=>el.placeholder=t(el.dataset.placeholder));
  renderFilters();renderProducts();renderGallery();renderCart();
}
function renderFilters(){
  const cats=["all",...new Set(data.products.map(p=>p.category))];
  $("#categoryFilters").innerHTML=cats.map(c=>`<button class="filter ${c===currentCategory?"active":""}" data-cat="${esc(c)}">${c==="all"?t("all"):esc(categoryLabel(c))}</button>`).join("");
  $$("#categoryFilters .filter").forEach(b=>b.onclick=()=>{currentCategory=b.dataset.cat;renderFilters();renderProducts()});
}
function renderProducts(){
  ...
  $("#productGrid").innerHTML=list.map(p=>`<article class="product-card">
    <div class="product-image"><img loading="lazy" src="${esc(p.image)}" alt="${esc(name(p))}">${p.oldPrice?`<span class="sale">${t("sale")}</span>`:""}</div>
    <div class="product-info"><div class="product-cat">${esc(categoryLabel(p.category))}</div><div class="product-name">${esc(name(p))}</div>
    <div class="price"><strong>€${Number(p.price).toFixed(0)}</strong>${p.oldPrice?`<span class="old-price">€${Number(p.oldPrice).toFixed(0)}</span>`:""}</div>
    <div class="product-actions"><button class="small-btn" data-details="${p.id}">${t("details")}</button><button class="small-btn primary" data-add="${p.id}">${t("add")}</button></div></div></article>`).join("");
  $$("[data-details]").forEach(b=>b.onclick=()=>openProduct(b.dataset.details));
  $$("[data-add]").forEach(b=>b.onclick=()=>addToCart(b.dataset.add));
}
function name(p){return p.name?.[lang]||p.name?.de||p.name}
function esc(s){return String(s??"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[m]))}
function renderGallery(){ $("#galleryGrid").innerHTML=data.gallery.map((src,i)=>`<div class="gallery-item"><img loading="lazy" src="${esc(src)}" alt="AT Evening Elegance ${i+1}"></div>`).join("") }
...
function openProduct(id){
  ...
  $("#productModalContent").innerHTML=`<div class="product-detail"><img src="${esc(p.image)}" alt="${esc(name(p))}"><div><p class="eyebrow">${esc(p.category)}</p><h2>${esc(name(p))}</h2><div class="price"><strong>€${Number(p.price).toFixed(0)}</strong>${p.oldPrice?`<span class="old-price">€${Number(p.oldPrice).toFixed(0)}</span>`:""}</div><p>${uiText("detail")}</p><div class="size-row">${["XS","S","M","L","XL"].map(s=>`<button class="size-btn">${s}</button>`).join("")}<button class="size-btn" data-custom>${uiText("custom")}</button></div><button class="btn btn-gold full" id="modalAdd">${t("add")}</button></div></div>`;
  $$("#productModalContent .size-btn").forEach(b=>b.onclick=()=>{$$("#productModalContent .size-btn").forEach(x=>x.classList.remove("selected"));b.classList.add("selected")});
  $("#modalAdd").onclick=()=>{addToCart(id);closeModal($("#productModalContent"))};
  openModal("#productModal");
}
function addToCart(id){cart.push(id);save();updateCartCount();openModal("#cartModal");renderCart()}
...
function renderCart(){
  ...
  $("#cartItems").innerHTML=items.length?items.map((p,i)=>`<div class="cart-row"><img src="${esc(p.image)}"><div><h4>${esc(name(p))}</h4><small>€${Number(p.price).toFixed(0)}</small></div><button class="cart-remove" data-remove="${i}">×</button></div>`).join(""):`<p class="modal-note">${uiText("empty")}</p>`;
  const total=items.reduce((s,p)=>s+Number(p.price),0);$("#cartTotal").textContent=`€${total.toFixed(0)}`;
  $$("[data-remove]").forEach(b=>b.onclick=()=>{cart.splice(Number(b.dataset.remove),1);save();renderCart();updateCartCount()});
}
function openRequest(){openModal("#requestModal")}
...
function sendOrderToWhatsApp(){
  ...
  if(!number){alert(lang==="de"?"Bitte zuerst die WhatsApp-Nummer im Editor eintragen.":lang==="en"?"Please add the WhatsApp number in the editor first.":lang==="ps"?"مهرباني وکړئ لومړی په اېډیټر کې د WhatsApp شمېره ولیکئ.":"لطفاً ابتدا شماره واتساپ را در ویرایشگر وارد کنید.");return;}
  window.open(`https://wa.me/${number}?text=${encodeURIComponent(msg)}`,"_blank");
}

function setup(){
  lang=localStorage.getItem("atEELang")||"de";
  $("#year").textContent=new Date().getFullYear();$("#languageSelect").value=lang;
  $("#languageSelect").onchange=e=>{lang=e.target.value;applyLang()};
  $("#menuButton").onclick=()=>$("#mainNav").classList.toggle("open");
  $$("#mainNav a").forEach(a=>a.onclick=()=>$("#mainNav").classList.remove("open"));
  $("#cartButton").onclick=()=>{renderCart();openModal("#cartModal")};
  $("#tailoringButton").onclick=openRequest;$("#ctaButton").onclick=openRequest;
  $("#orderButton").onclick=sendOrderToWhatsApp;
  $("#requestForm").onsubmit=e=>{e.preventDefault();const fd=new FormData(e.target);const items=cart.map(id=>data.products.find(p=>p.id===id)?.name?.[lang]).filter(Boolean).join(", ");const subject=encodeURIComponent(lang==="de"?"Bestellanfrage – AT Evening Elegance":"Order request – AT Evening Elegance");const body=encodeURIComponent(`Name: ${fd.get("name")}\nEmail: ${fd.get("email")}\nSize: ${fd.get("size")}\nProducts: ${items}\n\n${fd.get("message")}`);location.href=`mailto:${data.settings.email||"info@ateveningelegance.com"}?subject=${subject}&body=${body}`};
  $("#instagramLink").href=data.settings.instagram||"#";$("#tiktokLink").href=data.settings.tiktok||"#";updateCartCount();applyLang();applyBranding();
}
setup();
